...
def sigmoid(in:torch.DoubleTensor):
    return (1/(1+torch.exp(-in)))
...
class NeuralNetwork(object):
    def __init__(self, layers:list):
        ...
        for i in range(len(self.layers) - 1):
            self.theta[str(i)] = torch.from_numpy(np.random.normal(0, 1/sqrt(self.layers[i])), (self.layers[i]+1, self.layers[i+1]))
    ...
    def forward(self, input):
        ...
        output = input 
        bias = torch.ones((1,width))
        for i in range(len(self.layers) - 1):
            input_c = torch.cat(bias, output)
